...
class LSTMMask(BaseSKMObject, ClassifierMixin):

    def __init__(self, max_features=20000, maxlen=100, embedding_size=128, kernel_size=5, filters=64, pool_size=4,
                 lstm_output_size=70, batch_size=30, epochs=2):
        ...
        self.classifier.add(Embedding(max_features, embedding_size, input_length=maxlen))
        self.classifier.add(Dropout(0.25))
        self.classifier.add(Conv1D(filters, kernel_size, padding='valid', activation='relu', strides=1))
        self.classifier.add(MaxPooling1D(pool_size=pool_size))
        self.classifier.add(LSTM(lstm_output_size))
    ...
    def fit(self, X, y, classes=None, sample_weight=None):
        """ Calls the LSTM fit function from keras.
        Parameters
        ----------
    ...
    def partial_fit(self, X, y, classes=None, sample_weight=None):
        """ partial_fit
        Calls the LSTM partial_fit from keras.
        Parameters
        ----------
        X: numpy.ndarray of shape (n_samples, n_features)
            The feature's matrix.
    ...
    def predict(self, X):
        """ predict
        Uses the current classifier to predict samples in X.
        Parameters
        ----------
        X: numpy.ndarray of shape (n_samples, n_features)
            The feature's matrix.
